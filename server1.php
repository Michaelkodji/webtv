    ...
    if($req->rowCount()==0){
        ...
        while($dt=$req->fetch()){
            ...
            echo '<div class="col-sm-6">
                    ...
                    <div class="card-header"> '.$dt['prog_libelle'].'</div>
                    <div class="card-body">'.$dt['prog_description'].'</div>
                    <div class="card-footer text-muted">'.$dt['prog_date'].' à '.$dt['prog_hours'].' avec le pofesseur '.$dt['prog_prof'].'</div>
                </div>
                </div>';
        }
    }
        ...
        while($dt=$req->fetch()){
            ...
            echo '<div class="live-grid">
                    ...
                    <div class="live-card">
                      ...
                      <div class="card-body">
                        <h4 class="card-title">'.$dt['prog_date'].' à '.$dt['prog_hours'].' avec le pofesseur '.$dt['prog_prof'].'</h4>
                      </div>
                    ...
                    if ($dt['prog_date']>date("Y-m-d") && $dt['prog_hours']>date("H:d") ) {
                        echo'<div onclick="modif(\''.$data.'\')">&#9998;</div>
                        <div onclick="del_program(\''.$dt['prog_id'].'\')">&cross;</div>';
                    }
